...
const ProductCard = ({ product }) => {
  // Call the useUserStore hook here
  const { user } = useUserStore();
  const { addToCart } = useCartStore();

  const handleAddToCart = async () => {
    if (!user) {
      return toast.error("Please login to add products", { id: "login" });
    } else {
      // Add to the cart
      addToCart(product);
    }
  ...
  return (
    <div className="flex w-full relative flex-col overflow-hidden rounded-lg border border-gray-700 shadow">
